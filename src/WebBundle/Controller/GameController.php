...
class GameController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        // $game = $em->getRepository('AppBundle:Game')->findOneBy(array("title"=>$request->get('game')));
        ...
        // if ($game != null) {
        //         $repo_query->leftJoin('p.games', 'g');
        //         $repo_query->andWhere('g.id = ' . $game->getId());
        // }

        // $repo_query->andWhere("p.type like 'game'");
    ...
    public function filterAction(Request $request)
    {
        ...
        return $this->render('WebBundle:Game:filter.html.twig',
            array(
                "posters"=>$posters,
                "games"=>$games
            )
    ...
    public function shareAction($id)
    {
        ...
        $query = $repository->createQueryBuilder('p')
            ->leftJoin('p.genres', 'g')
            ->where("p.enabled = true","p.type like 'game' ",'g.id in ('.$genres.')')
            ->addSelect('RAND() as HIDDEN rand')
            ->orderBy('rand')
            ->setMaxResults($nombre)
            ->getQuery();
        $related_posters = $query->getResult();
        $layout  = ($poster->gettype() == "game")?'WebBundle:Game:view.html.twig':'WebBundle:Serie:view.html.twig';
    ...
    public function viewAction($id,$slug)
    {
        ...
        $poster = $em->getRepository('AppBundle:Poster')->findOneBy(array("type"=>"game","id"=>$id,"slug"=>$slug,"enabled"=>true));
        if ($poster == null) {
            throw new NotFoundHttpException("Page not found");  
        }
        ...
        return $this->render('WebBundle:Game:view.html.twig',array(
            "poster" => $poster,
            "comments"=>$comments,
            "ratings"=>$ratings,
            "related_posters"=>$related_posters,
    ...
    {       
        ...
        return $this->render('WebBundle:Game:subtitles.html.twig',array(
            "poster"=>$poster
        ));
    }
    ...
    {     
        ...
        return $this->render('WebBundle:Game:downloads.html.twig',array(
            "sources"=>$sources,
            "premium"=>$premium,
        ));
    }
